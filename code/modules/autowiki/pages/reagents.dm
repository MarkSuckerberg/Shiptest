...
/datum/autowiki/reagents/generate()
	...
	for(var/category in sortList(categories))
		output[category] = generate_category(categories[category])

	return output

/datum/autowiki/reagents/proc/generate_category(list/datum/reagent/reagents)
	var/output = ""
	...
	for(var/datum/reagent/reagent as anything in reagents)
		output += "! style='background-color: #FFEE88;' | <span id='[reagent.name]'>[escape_value(reagent.name)]</span> <span style='color:[escape_value(reagent.color)];background-color:[escape_value(reagent.color)]'>__</span>\n"
